  ...
  deleteUser( firstName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection('users').deleteOne({"firstName" : firstName}, null, callback);
  ...
  deleteProduct( productName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection('products').deleteOne({"name" : productName}, null, callback);
  ...
  addProductToShoppingCart({userFirstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('products').find({"name" : productName}).toArray((error, product)=> {
          database.collection('users').update({'firstName' : userFirstName}, {$push : {"shoppingCart" : product[0]._id }},null, callback);
        });
  ...
  addReviewToProduct( {productName, review}, callback = (messageResult) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('products').update({"name" : productName}, {$push : {"reviews" : review}}, null, callback);
      }
    });
  }
}
