  ...
  insertProduct(product, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('products').insertOne({ product: product }, callback);
  ...
  addProductToShoppingCart({ userFirstName, productName }, callback = error => {}) {
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('users').findOne({ firstName: userFirstName }, { $push: { name: productName } }, callback);
