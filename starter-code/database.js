  ...
  insertUser(user, mainCallback){
    this.connect((error, database) => {
      if (error){
        ...
        database.collection('users').insertOne(user, (err, elonmusk) => {
          // 1. show dbs
          // 2. use xxxxxxx
          // 3. db.whatevercollectioninsideofthedb.insertOne({Object})
          //database.collection('whatevercollectioninsideofthedb').insertOne()
          mainCallback(err, elonmusk);
        });
      }
    });
  }
