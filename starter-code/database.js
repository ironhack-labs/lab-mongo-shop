...
class Database {
  ...
  addProductToShoppingCart({userFirstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      ...
      //  database.collection(users).find(userFirstName)
      //  database.collection(products).find(productName)
       database.collection(users).updateOne({"firstName": userFirstName}, {$push:{"shoppingCart": productName}}, callback)
        // database.collection(users).find(userFirstName).next(database.collections(users).insertOne({productName}, callback))
