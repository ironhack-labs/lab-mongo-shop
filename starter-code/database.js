  ...
  insertUser(user, callback = (error, result) => {}){
    this.connect((error, database) => {
      if (error){
        ...
        // remember once it's finished to comment callback('Error inserting user');
      //  database.collection(users).insertOne(user, callback);
//        callback('Error inserting user');
      database.collection('user').insertOne(user, (err, newDoc) => {
          callback(err, newDoc);
      });
  ...
  listUsers(callback = (error, users) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        //  LAB 2
        // Implement the query to insert a user
        // remember once it's finished to comment callback('Error listing users');
//        callback('Error listing users');
          database.collection('user').find({}, {}, callback);
      }
    });
  }
  ...
  deleteUser( firstName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        // remember once it's finish to comment callback('Error deleting user');
//        callback('Error deleting user');
          database.collection('user').deleteOne({ "firstName": firstName } , callback);
  ...
  insertProduct(product, callback = (error, result) => {}){
    this.connect((error, database) => {
      if (error){
        ...
        // remember once it's finish to comment callback('Error inserting product');
  ...
  listProducts(callback = (error, products) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        // remember once it's finish to comment callback('Error listing products');
  ...
  deleteProduct( productName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        // remember once it's finish to comment callback('Error deleting product');
//        callback('Error deleting product');
        database.collection('product').deleteOne({ "name": productName } , callback);
      }
    });
  }
  ...
  addProductToShoppingCart({userFirstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        // remember once it's finish to comment callback('Error buying product');
//        callback('Error buying product');
        database.collection('user').find({ "firstName": userFirstName}).toArray(callback);
      }
    });
  }
  ...
  addReviewToProduct( {productName, review}, callback = (messageResult) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        // remember once it's finish to comment callback('Error reviewing product');

//        callback('Error reviewing product');
        database.collection('product').find({"name": productName}).toArray(callback);
      }
    });
  }
}
