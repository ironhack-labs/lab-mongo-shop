  ...
  insertUser(user, callback = (error, result) => {}){
    this.connect((error, database) => {
      if (error){
        ...
        database.collection("users").insertOne(user, callback);
        // database.collection("users").updateOne({"firstName":  })
  ...
  listUsers(callback = (error, users) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection("users").find().toArray((errorMongo, users) => {
          callback(errorMongo, users); // Thom: null swappable with error.
        });
  ...
  deleteUser( firstName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection("users").deleteOne(user, callback);
  ...
  listProducts(callback = (error, products) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection("products").find().toArray((errorMongo, products) => {
          callback(errorMongo, products);
        });
  ...
  deleteProduct( productName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection("products").deleteOne(products, callback);
  ...
  addProductToShoppingCart({userFirstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection("products").find({"name": productName }).toArray((error, products) => {

          database.collection("users").updateOne(
            { "firstName": userFirstName },
            { $push: {"shoppingCart": products[0] }}, null);
          callback(null);
        });
  ...
  addReviewToProduct( {productName, review}, callback = (messageResult) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        // database.collection("users").findOne("firstName": firstName)

        database.collection("products").updateOne(
          { "name": productName },
          { $push: {"reviews": review }}, null);
          callback(null);
