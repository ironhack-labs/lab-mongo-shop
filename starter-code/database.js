  ...
  deleteProduct( productName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection('products').deleteOne({"product.name":productName},callback); 
  ...
  addProductToShoppingCart({userFirstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      if (error) {
       ...
       var productId =  database.collection('products').find({"name":productName},{"_id":1})

        database.collection('users').updateOne(
          { "firstName": userFirstName},
          { $set: { "shoppingCart": [productId._id] } }
        ); 
