...
class Database {
  ...
  listUsersDisplay(callback = (error, users) => {}) {
    this.connect((error, database) => {
      if (error) {
        callback(error);
      } else {
        ...
        callback(null, database.collection("users").find());
  ...
  deleteUser(firstName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error) {
        ...
        callback(null, database.collection("users").deleteOne(user, callback));
  ...
  listProducts(callback = (error, products) => {}) {
    this.connect((error, database) => {
      if (error) {
        ...
        callback(null, database.collection("products").find());
  ...
  deleteProduct(productName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error) {
        ...
        callback(
          null,
          database.collection("products").deleteOne(productName, callback)
        );
      ...
      if (error) {
        ...
        callback(
          ...
          database
            .collection("users")
            .find("userFirstName")
            .insertOne(productName, callback)
        );
