  ...
  deleteUser( firstName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection('users').deleteOne({'firstName': firstName});
        callback('User named ' + firstName + ' deleted');
      }
    });
  }
  ...
  insertProduct(product, callback = (error, result) => {}){
    this.connect((error, database) => {
      if (error){
        ...
        callback( product.name + ' inserted');
      }
    });
  }
  ...
  deleteProduct( productName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection('products').deleteOne({'name': productName});
        callback('Product ' + productName + ' deleted');
      }
    });
  }
  ...
  addProductToShoppingCart({userFirstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('users').updateOne({"firstName": userFirstName}, { $push: { "shoppingCart": productName }});
        callback(userFirstName + ' bought ' + productName);
      }
    });
  }
  ...
  addReviewToProduct( {productName, review}, callback = (messageResult) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection("products").updateOne({"name": productName}, {$push: {"reviews": review}});
        callback('Review added');
      }
    });
  }
}
