...
class Database {
  ...
  deleteUser( name, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error){
        callback(error);
      } else {
        ...
        database.collection('users').deleteOne({firstName: name}, callback);
  ...
  addReviewToProduct( {productName, review}, callback = (messageResult) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('products').updateOne({name: productName}, {$set : {reviews : review}}, callback);
