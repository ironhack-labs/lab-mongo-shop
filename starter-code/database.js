  ...
  addProductToShoppingCart({userFirstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('users').updateOne({"firstName":userFirstName}, {$set:{"shoppingCarts.name": productName}});
  ...
  addReviewToProduct( {productName, review}, callback = (messageResult) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('products').updateOne({"name":productName}, {$set:{"reviews": review}});
