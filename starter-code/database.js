...
class Database {
  ...
  listUsers(callback = (error, users) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection('users').find(users.firstName, callback);
        // Implement the query to insert a user
  ...
  deleteUser( firstName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection('users').deleteOne(users.firstName, callback);
        //  LAB 3
  ...
  listProducts(callback = (error, products) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection('products').find(products.productName, callback);
        // LAB 5
    ...
    this.connect((error, database) => {
      if (error){
        callback(error);
         ...
         database.collection('products').deleteOne(products.productName, callback);
        // LAB 6
  ...
  addProductToShoppingCart({userFirstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      ...
      database.collection('users','products').find(users.firstName, products.productName, callback);
        // LAB 7
  ...
  addReviewToProduct( {productName, review}, callback = (messageResult) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('products').find(products.productName, products.review, callback);
        // LAB 8
