  ...
  listUsers(callback = (error, users) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection('users').find().toArray( (error, array) => {
  ...
  addProductToShoppingCart({userFirstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        // let product = database.collection('products').find({'name':productName})
        // database.collection('users').update({'firstName':userFirstName}, {$push: {"shoppingCart": product}})
        
        database.collection('products').findOne({'name': productName}, (error, result) => {
          database.collection('users').update({'firstName':userFirstName}, {$push: {"shoppingCart": result}})
