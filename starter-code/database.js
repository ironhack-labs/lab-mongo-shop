  ...
  listUsers(callback = (error, users) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        //  LAB 2
        // Implement the query to list all users
  ...
  deleteUser( firstName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error){
       ...
       var myQuery = {"firstName" : firstName}
        database.collection('users').deleteOne(myQuery, callback)
        database.close();
  ...
  deleteProduct( productName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        var myQuery = {"name" : productName}
        database.collection('products').deleteOne(myQuery, callback)
  ...
  addProductToShoppingCart({userFirstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('users').findOneAndUpdate({firstName: userFirstName}, {$push: { shoppingList : productName}}, callback)
        
        // var shop = database.collection('users').updateOne({"firstName" : userFirstName}, {$push : {"shoppingCart": pro._id}} , callback)
  ...
  addReviewToProduct( {productName, review}, callback = (messageResult) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('products').findOneAndUpdate({name : productName}, {$push: {reviews: review}}, callback)
