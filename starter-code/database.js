  ...
  deleteUser( firstName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection(users).remove({name : firstName},callback);
  ...
  addProductToShoppingCart({userFirstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection(shoppingCarts).find({name:userFirstName},callback).insert({name:productName},callback);
  ...
  addReviewToProduct( {productName, review}, callback = (messageResult) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection(products).updateOne({name:productName}, {review},callback);
