  ...
  addProductToShoppingCart({userFirstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('users').updateOne({firstName: userFirstName}, {name: productName}, callback);
      }
    });
  }
    ...
    this.connect((error, database) => {
      if (error) {
        ...
        // remeber once it's finish to comment callback('Error reviewing product');
          ...
          database.collection('users').updateOne({name: productName}, review, callback);
      }
    });
  }
}
