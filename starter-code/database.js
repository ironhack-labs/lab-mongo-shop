  ...
  addProductToShoppingCart({userFirstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('products').find({"name" : productName}).toArray((error, product) => {                    database.collection('users').update({'firstName' : userFirstName}, {$push : {"shoppingCart" : product[0]._id}},null, callback);
