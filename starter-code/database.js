...
// Remember that purchases are the elements inside the 
// "shoppingCart" array property of a user.
  ...
  listUsers(callback = (error, users) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        //  LAB 2
        // Implement the query to insert a user
        // remember once it's finish to comment callback
        // ('Error listing users');
  ...
  deleteProduct( productName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        // productName is the name of the product to to delete 
  ...
  addReviewToProduct( {productName, review}, callback = (messageResult) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('products').update({name:productName}, {$push:{reviews:review}});
