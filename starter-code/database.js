  ...
  addProductToShoppingCart({userFirstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        var prod = database.collection('products').find({productName},callback);
        console.log('done');
        database.collection('users').update({firstName:userFirstName},{$push:{'shoppingCart':prod}},callback);
