  ...
  deleteUser( firstName, callback ) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection('users').deleteOne(filter, callback);
      }
    });
  }
  ...
  deleteProduct( productName, callback = (error, result) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection('products').deleteOne(filter, callback);
      }
    });
  }
  ...
  addProductToShoppingCart({userFirstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('users').updateOne(filter, updateTarget, callback);
      }
    });
  }
  ...
  addReviewToProduct( {productName, review}, callback = (messageResult) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('products').updateOne(filter, updateTarget, callback);
      }
    });
  }
}
