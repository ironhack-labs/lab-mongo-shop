  ...
  addProductToShoppingCart({userFirstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('users').update({'firstName' : userFirstName}, {$push : {"shoppingCart" : product[0]._id }}, callback);
      });
  ...
  addReviewToProduct( {productName, review}, callback = (messageResult) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection('products').updateOne({name: productName},{$push:{"reviews":{"name":review.name,"comment":review.comment,"stars":review.stars,"date":review.date}}}, callback);
