  ...
  insertUser(user, callback = (error, result) => {}){
    this.connect((error, database) => {
      if (error){
        ...
        this.database.collection("users").insertOne(user, callback); // {"firstName": "","lastName":"","dateBirth":"","adress":{"streetAdress":"", "city":"", "state":"","postalCode":""}}
  ...
  listUsers(callback = (error, users) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        this.database.collection("users").find({}).pretty();
  ...
  listProducts(callback = (error, products) => {}) {
    this.connect((error, database) => {
      if (error){
        ...
        database.collection("products").listProducts();
